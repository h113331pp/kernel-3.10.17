...
static char *wol_kernel_cmdline;
static int wol_kernel_cmdline_setup(char *s)
{
	wol_kernel_cmdline = s;
	return 1;
}
__setup("wol=", wol_kernel_cmdline_setup);

static int
rtl8168_set_wol_kernel_cmdline(struct net_device *dev)
{
    struct rtl8168_private *tp = netdev_priv(dev);
    unsigned long flags;
    ...
    if ( (wol_kernel_cmdline != NULL) && 0==strcmp("off", wol_kernel_cmdline) )
    {
        printk("boot wol disabled\n");
        tp->wol_opts = WOL_DISABLED;
...
static void rtl8168_set_custom_led(struct rtl8168_private *tp);
                 ...
                 const struct pci_device_id *ent)
{
    ...
    rtl8168_set_wol_kernel_cmdline(dev);
